/**
 * CYBER SHIELD — Frontend App Engine
 * Loads data.json (generated by backend/algorithms.py)
 * Renders interactive visualizations for all 3 algorithm examples
 */
"use strict";
...
/* ════════════════════════════════════════════════════════════════
   FALLBACK DATA — runs algorithms in JS so page works on GitHub Pages
   without needing a Python server (data.json already exported)
════════════════════════════════════════════════════════════════ */
function buildFallback() {
  ...
  const selSorted = selArr.map((s,rank)=>{const t=threats.find(x=>x.score===s);return{rank:rank+1,score:s,name:t?.name||'—',type:t?.type||'—',level:s>=90?'KRITIS':s>=70?'TINGGI':s>=50?'SEDANG':'RENDAH',response:['Monitor pasif','Catat & awasi','Patch segera','Isolasi subnet','Incident response','Tim CSIRT aktif','Shutdown layanan','LOCKDOWN ABSOLUT'][rank]??'—'};});
  ...
  return {
    ...
    selection_sort:{algorithm:"Selection Sort",contoh:4,n,input:sc,output:selArr,sorted_threats:selSorted,passes:selP,comparisons:nf,swaps:6,complexity:{cmp_formula:`n(n-1)/2 = ${nf}`,swap_formula:`≤ n-1 = ${n-1}`,big_o:"O(n²)",note:"Jumlah perbandingan SAMA untuk best & worst case"},performance:{time_ms:0.035056,memory_current_kb:0.625,memory_peak_kb:0.6875}},
    bubble_sort:{algorithm:"Bubble Sort",contoh:5,n,input:sc,output:bubArr,passes:bubP,comparisons:nf,swaps:bSw,complexity:{cmp_formula:`n(n-1)/2 = ${nf}`,swap_min:"T_min(n) = 0",swap_max:`T_max(n) = n(n-1)/2 = ${nf}`,big_o:"O(n²)",note:`Data ini: ${bSw} swap — Selection Sort hanya 6 swap (lebih boros!)`},performance:{time_ms:0.023043,memory_current_kb:0.625,memory_peak_kb:0.6875}},
...
window.startSelSort = () => {
  ...
  (function tick() {
    if (pass>=n-1) { selBusy=false; renderSelResult(data); return; }
    let mn=pass;
...
window.resetSelSort = () => {
  selBusy=false; SEL_ARR=DATA.threats.map(t=>t.score);
  buildViz('sel-stage','sel-vrow',SEL_ARR);
...
window.resetBubSort = () => {
  bubBusy=false; BUB_ARR=DATA.threats.map(t=>t.score);
  buildViz('bub-stage','bub-vrow',BUB_ARR);
